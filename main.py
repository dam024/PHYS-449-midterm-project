...
def plot_results(obj_vals, cross_vals, res_path):
    ...
    plt.plot(range(len(obj_valsFlat)), obj_valsFlat, label= "Generator : ", color="blue")
    plt.plot(range(len(cross_valsFlat)), cross_valsFlat, label= "Critic : ", color= "green")
    plt.legend()
...
def main(prefix):
	...
	parser.add_argument('-p','--param',help='Path to json file containing the parameters for the program. See example at default location.',default=prefix+'parameters/param_used.json')
	...
	#Create network
	network  = NN.NeuralNetwork(param['NN_structure'],param['training'], args.isTraining,args.model,args.resumeTraining,lossPath)
