...
class NeuralNetwork:
    ...
    def __init__(self, params, trainingParams, isTraining, modelSavingPath, resumeTraining, lossPath):
        ...
        self.optimizerGenerator = torch.optim.Adam(
            self.generator.parameters(), lr=trainingParams['learning_rate_generator'])
        self.optimizerCritic = torch.optim.Adam(
            self.critic.parameters(), lr=trainingParams['learning_rate_critic'])
    ...
    def trainNetwork(self, inputManager, params, modelSavingPath):
        ...
        for epoch in range(self.epoch, params['epoch']):
            ...
            generated = self.forward(data.x).to(NeuralNetwork.device())
        ...
        # To stop at the correct epoch in case of success
        self.epoch = params['epoch']
        self.print("End of training \n")
        if len(self.obj_vals['critic']) > 0 and len(self.obj_vals['generator']) > 0:
            self.print('Final training results : \tGenerator: {:.4f} \tCritic: {:.4f}'.format(
    ...
    def saveParameters(self, path, lossPath):
        ...
        if extension != ".pt":
            ...
            self.print("Error : File at "+path +
                       " should have extension .pt But don't worry, I thought to that case and I created at this path : "+newPath)
            path = newPath
    ...
    def getParameters(self, path, resumeTraining, lossPath):
        ...
        self.optimizerGenerator.load_state_dict(
            state_dict[self.__generatorOptimizerKey])
